...
def test_LoadPostgresqlMicrographUsesThumbnailProjectContext(
    service,
    currentProject,
    projectService,
    mapper,
    ...
    micrograph = service._loadPostgresqlMicrograph(
        mapper=mapper,
        projectId=1,
        currentUser=currentUser,
        protocolId=500,
    ...
    assert projectService.loadProjectForThumbnailsCalls == [
        {
            "dbProj": projectService.projectRow,
            "mapper": mapper,
        }
    ]
    assert projectService.runtimeCalls == [
...
def test_RenderMicrographImageUsesPostgresqlMicrographLoader(
    coords2dServiceModule,
    service,
    mapper,
    currentUser,
    monkeypatch,
):
    micrograph = FakeMicrograph(
        objId=1,
        location=(0, "/data/micrograph-a.mrc"),
        micName="Micrograph A",
        dims=(32, 24),
    ...
    def failLegacyLoader(*args, **kwargs):
        raise AssertionError("Legacy coordinates loader must not be used")

    monkeypatch.setattr(
        service,
        "_loadPostgresqlMicrograph",
        loadPostgresqlMicrograph,
    )
    monkeypatch.setattr(
        service,
        "_loadCoordinatesOutput",
        failLegacyLoader,
    )
    monkeypatch.setattr(
        coords2dServiceModule.os.path,
        "exists",
        lambda path: True,
    )
    monkeypatch.setattr(
        service,
        "_readMicrographImage",
        lambda imagePath, imageIndex: coords2dServiceModule.Image.new(
            "L",
            (32, 24),
            128,
        ),
    ...
    assert response.headers["X-Preview-Original-Width"] == "32"
    assert response.headers["X-Preview-Original-Height"] == "24"
    assert postgresqlCalls == [
        {
            "mapper": mapper,
            "projectId": 1,
            "currentUser": currentUser,
