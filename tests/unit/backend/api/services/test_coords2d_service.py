...
def test_ListMicrographsResolvesPostgresqlProtocolId(
    service,
    currentProject,
    projectService,
    mapper,
    ...
    protocol = FakeProtocol(objId=10)
    protocol.outputCoordinates = buildCoordinatesOutput()

    currentProject.protocols[10] = protocol
    projectService.runtimeProtocolIdByDbId[500] = 10

    result = service.listMicrographs(
        mapper=mapper,
        projectId=1,
        currentUser=currentUser,
        protocolId=500,
        ...
        "micrographs": [
            {
                "id": "1",
                "index": 1,
                "fileName": "/data/micrograph-a.mrc",
                "label": "Micrograph A",
                "particles": 2,
                "updated": False,
                "width": 4096,
            ...
            },
            {
                "id": "2",
                "index": 2,
                "fileName": "/data/micrograph-b.mrc",
                "label": "Micrograph B label",
                "particles": 1,
                "updated": False,
                "width": 2048,
...
def test_ListCoordinatesForMicrographResolvesPostgresqlProtocolId(
    service,
    currentProject,
    projectService,
    mapper,
    ...
    result = service.listCoordinatesForMicrograph(
        mapper=mapper,
        projectId=1,
        currentUser=currentUser,
        protocolId=500,
...
def test_ListCoordinatesForMicrographRaisesWhenMicrographIsMissing(
    service,
    currentProject,
    projectService,
    mapper,
    ...
    with pytest.raises(HTTPException) as exc:
        service.listCoordinatesForMicrograph(
            mapper=mapper,
            projectId=1,
            currentUser=currentUser,
            protocolId=500,
    ...
    assert exc.value.status_code == 404
    assert exc.value.detail == "Micrograph '999' not found in coordinates output"
