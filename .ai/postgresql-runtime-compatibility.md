...
40. PostgreSQL restart validation must confirm that the current project exposes its PostgreSQL runtime mapper before any persisted-output cleanup or input-ref cleanup begins. Runtime-mapper availability is a restart precondition, not a launch-time failure; the launch path may retain a defensive recheck, but a missing runtime mapper must be reported during validation while the subtree is still untouched.
41. PostgreSQL continue planning must confirm that the current project exposes its PostgreSQL runtime mapper before any persisted-output cleanup or input-ref cleanup for restart-classified protocols begins. Runtime-mapper availability is a continue-plan precondition; a missing mapper must make the plan invalid while the mixed subtree is still untouched, while the launch path may retain its defensive recheck.
42. PostgreSQL output cleanup paths that receive a Scipion runtime protocol object must treat `protocol.getObjId()` exclusively as `protocols.protocolId`. If that runtime protocol id is absent from PostgreSQL, cleanup must be skipped; it must never reinterpret the known Scipion id as `protocols.id` and risk deleting another protocol's outputs. Dual protocol-id resolution remains valid only for cleanup boundaries that receive an id without a runtime protocol object.
43. PostgreSQL child input-reference cache invalidation paths that receive Scipion runtime protocol objects must resolve each `protocol.getObjId()` exclusively through `protocols.protocolId`. If that runtime protocol id is absent, its child-reference cleanup must be skipped; the known Scipion id must never fall back to `protocols.id`, which could clear cached output object ids belonging to children of an unrelated protocol.
44. PostgreSQL reset validation must successfully snapshot every candidate protocol's owned output names and runtime pointer-parameter structure before stopping workers or deleting persisted outputs and child input-reference caches. Failures from `iterOutputAttributes()`, `getDefinition()` or `iterParams()` are validation failures and must abort the whole reset while the subtree is untouched; the later reset phase must consume the validated snapshot instead of re-enumerating mutable runtime structure.
45. PostgreSQL subtree reset must scope persisted-output cleanup and child input-reference cache invalidation to each protocol immediately before that protocol is reset. Validation and active-worker stopping remain whole-subtree barriers, but a later protocol must never lose its outputs or cached child references merely because an earlier protocol reset failed before execution reached it.
46. PostgreSQL restart validation must successfully snapshot every candidate protocol's owned output names and runtime pointer-parameter structure before persisted-output cleanup or child input-reference cache invalidation begins. Failures from `iterOutputAttributes()`, `getDefinition()` or `iterParams()` must invalidate the whole restart while the subtree is untouched, and the restart launcher must consume the validated snapshot instead of re-enumerating runtime structure after cleanup.
47. PostgreSQL continue planning must successfully snapshot owned output names and runtime pointer-parameter structure for every protocol classified for restart before persisted-output cleanup or child input-reference cache invalidation begins. Failures from `iterOutputAttributes()`, `getDefinition()` or `iterParams()` must invalidate the continue plan while the mixed subtree is untouched, and restart preparation must consume that validated snapshot instead of re-enumerating runtime structure after cleanup. Resume- and skip-classified protocols must not be required to expose restart-only structural metadata.
48. PostgreSQL continue execution must scope persisted-output cleanup and child input-reference cache invalidation to each restart-classified protocol immediately before that protocol's restart preparation. The complete mixed continue plan must still be validated before any destructive operation begins, but a restart-classified protocol that execution never reaches must never lose its outputs or cached child references because an earlier restart or resume preparation failed.
49. PostgreSQL restart execution must scope persisted-output cleanup and child input-reference cache invalidation to each protocol immediately before that protocol's restart preparation. Whole-subtree restart validation remains a mandatory barrier before destructive work begins, but protocols that execution never reaches must retain their persisted outputs and cached child references when an earlier protocol preparation fails.
...
54. PostgreSQL runtime protocol persistence must store the protocol row and its protocol-step replacement within one PostgreSQL transaction. `saveProtocol()` and `replaceProtocolSteps()` must be able to participate without committing independently, so any failure while replacing protocol steps rolls back the corresponding protocol metadata update instead of exposing a protocol row and step snapshot from different persistence generations.
55. Nested `PostgresqlDb.transaction()` scopes on the same thread must participate in one outer PostgreSQL transaction. Entering or successfully leaving an inner transaction scope must never commit independently, and an exception propagated through nested scopes must be rolled back by the outermost transaction boundary. Transaction nesting depth must remain thread-local and must be restored after both successful and failed transaction scopes.
56. PostgreSQL database primitives must respect an active explicit `PostgresqlDb.transaction()` scope. `execute()` and `executeReturningOne()` must never commit independently while the current thread is inside an explicit transaction, and statement/read failures inside that scope must propagate to the transaction boundary instead of performing an independent rollback. Outside an explicit transaction, their existing immediate commit/rollback behavior remains unchanged.
...
58. PostgreSQL dependency scheduling must distinguish streaming capability from streaming input consumption. A child may consume an output while its parent is still active only when the child both reports streaming support and implements runtime input refresh through `_stepsCheck`; protocols that merely inherit or override `worksInStreaming()` without incremental input refresh must wait until the input parent finishes.
