...
class ProtocolGraphRepository:
    ...
    def resolvePersistedRelationEndpoint(
            self,
            mapper,
            projectId: int,
        ...
        """
        Resolve a relation endpoint using its runtime id first.

        When a source output was recreated and its runtime id changed,
        use the creator protocol input refs to recover the authoritative
        parent protocol and output name.
        """
        persistedObject = (
            self
        ...
        inputRefs = mapper.db.fetchAll(
            ...
            SELECT DISTINCT
             ...
             WHERE "projectId" = %s
               AND "protocolDbId" = %s
               AND "objectId" = %s
               AND "parentProtocolDbId" IS NOT NULL
               AND COALESCE(
                    "parentOutputName",
                    ''
               ) <> ''
        ...
        semanticEndpoints = {
            (
                int(row["parentProtocolDbId"]),
                str(row["parentOutputName"]).strip(),
            )
            for row in inputRefs or []
            if (
                    row.get("parentProtocolDbId")
                    not in (None, "",)
                    and str(
                        row.get("parentOutputName")
                        or ""
                    ).strip()
            )
        }

        if len(semanticEndpoints) != 1:
            return None

        parentProtocolDbId, outputName = next(
            iter(semanticEndpoints)
        )
